...
- **플래시 메모리(Flash memory)** : 가장 최근에 나타난 EEPROM 유형의 매체. RAM처럼 원하는 위치를 마음대로 읽을 수 있다. 하지만 빈 플래시 메모리에 데이터를 기록하기 위해서는 먼저 0을 채워 넣어야 한다. 플래시 메모리는 읽을 때는 임의 접근(random access) 장치이고 쓸 때는 블록 접근(block access) 장치이다.
