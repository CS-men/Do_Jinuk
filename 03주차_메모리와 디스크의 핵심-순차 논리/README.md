...
: 훨씬 더 많은 정보를 저장해야 한다면, 레지스터를 많이 쌓아두는 것부터 생각해 볼 수 있다. 그리고, 그 레지스터마다 번호(주소, address)를 부여한다. 그 후, 지정한 주소에 해당하는 레지스터의 출력을 선택할 방법이 필요하다. 이는 실렉터로 해결할 수 있다.

- **임의 접근 메모리**
  - RAM(=Random Access Memory) : 메모리 위치 중 원하는 곳은 어디든 원하는 순서로 쓰거나 읽을 수 있게 한다.
  - SRAM(=Static RAM) : 비싸지만 아주 빠르다.
  - DRAM(=Dynamic RAM) : 버킷에 주기적으로 전자를 다시 채워 넣어야 한다. 또한 집적도가 높기 때문에 큰 메모리 칩에 사용된다.
  - SRAM과 DRAM은 모두 휘발성 메모리다.

- **읽기 전용 메모리**
  - ROM(=Read-Only Memory) : 한번만 쓰고 여러 번 읽을 수 있는 메모리
  - Mask-programmable ROM : 마스크 프로그래머블 ROM은 임의 접근이 가능하다.
  - PROM(=Programmable ROM) : PROM은 직접 프로그래밍할 수 있는 ROM으로, 단 한 번만 ROM에 프로그램을 적어 넣을 수 있다.
  - EPROM(=Erasable programmable ROM) : 특별한 자외선 빛 아래 EPROM을 넣어두면 저장된 내용을 지울 수 있었다.
  - EEPROM(=Electrically EPROM) : 전기로 지울 수 있는 EPROM으로, EEPROM은 내부 데이터를 아무 순서로나 쓰고 읽을 수 있기 때문에 기술적으로는 RAM이다. 하지만, 데이터를 쓰는 데 시간이 오래 걸리고 RAM보다 비싸기 때문에 ROM을 대신하는 목적으로 쓰인다.
...
- 디스크 드라이브(Disk drive) : 대량 저장장치로 알려져 있으며, 엄청나게 많은 데이터를 저장하기 아주 좋은 장치다. 또한, 상대적으로 다른 유형의 메모리에 비해 느리다.
- **하드 드라이브(hard drive)** : 디스크 드라이브를 하드 드라이브라고 불렀다. 이후, 플로피 디스크(floppy disk)라는 더 저렴하고 탈착이 가능한 드라이브가 등장하면서 구분이 생겼다.
- **광학 디스크(optical disk)** : 자기 디스크와 비슷하지만 데이터를 읽고 쓰기 위해 자성 대신 빛을 사용한다는 차이가 있다.
...
- **플래시 메모리(Flash memory)** : 가장 최근에 나타난 EEPROM 유형의 매체. RAM처럼 원하는 위치를 마음대로 읽을 수 있다. 하지만 빈 플래시 메모리에 데이터를 기록하기 위해서는 먼저 0을 채워 넣어야 한다. 플래시 메모리는 읽을 때ㅔ는 임의 접근(random access) 장치이고 쓸 때는 블록 접근(block access) 장치이다.
- 디스크 드라이브는 점차 **SSD**로 널리 알려진 고체 상태 드라이브(Solid-State Drive)로 교체되고 있다.
...
- **패리티(parity)**를 사용하면 단 1비트만 데이터가 잘못된 경우를 감지할 수 있다.
- 짝수 패리티(even parity)는 모든 비트를 서로 XOR 한 값을 사용한다.
- 홀수 패리티(odd parity)는 XOR 한 값의 보수를 사용한다.
...
- 논리를 하드웨어로 만드는 것과 소프트웨어로 만드는 것 사이에서 차이를 찾는다는 것은 모호하다.
  - 상당 부분, 소프트웨어는 설계에 들어가는 시간이라는 비용을 제외한 어떤 추가 비용도 들지 않기 때문에 소프트웨어가 하드웨어보다 훨씬 더 만들기 쉽다는 점을 빼고는 차이가 거의 없다.
...
-   책을 읽고 지식을 추가한다는 것에 있어서 적지 않은 문제점을 찾을 수 있었다. 그 중 무엇보다도, 현재 면접에 준비하는 자세에서는 조금 거리가 있지 않나 싶었다. 그래서 스터디원들과 힘을 합쳐 면접에서 더 많이 요구하는 질문에 대해서 준비해보려고 한다.